...
class KegiatanPengeluaran extends Model {
    protected string $table = 'kegiatan_pengeluaran';
    ...
    public function getTotalByKegiatan(int $kegiatanId): float {
        $result = $this->query("SELECT COALESCE(SUM(jumlah), 0) as total FROM kegiatan_pengeluaran WHERE kegiatan_id = :kid", [':kid' => $kegiatanId]);
