<?php
class DonasiPemasukan extends Model {
    protected string $table = 'donasi_pemasukan';
    ...
    public function getTotalByProgram(int $programId): float {
        $result = $this->query("SELECT COALESCE(SUM(jumlah), 0) as total FROM donasi_pemasukan WHERE program_id = :pid", [':pid' => $programId]);
